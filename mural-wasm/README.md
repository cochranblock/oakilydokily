...
- **Fallback**: CC0 pixel art (CatnDog + embedded guinea pig) 4×3 sprite sheet when claymation not available
...
`build-standalone.sh` runs the Python claymation pipeline when `rembg` is installed. Output: `claymation_spritesheet.png`, `claymation_meta.json`. Falls back to `pets_spritesheet.png` (CC0: CatnDog pzUH + embedded guinea pig) otherwise.
